...
def is_head():
    ...
    probability = 0.5
    # use binomial distribution (returns 0 or 1)
    ...
    if result == 1:
        ...
        delay()
        print('Próximo jogador passa a vez')
        return False
...
def main():
    ...
    print(' 7 │ 8 │ 9 ')
    print('')
    print('Regras adicionais: entre cada jogada, uma moeda é lançada. Se a moeda')
    print('der “cara”, o jogador seguinte joga, caso contrário ele passa a vez')
    print('')
    ...
    if human_goes_first:
        ...
        computer_goes_next = True
    else:
        human, computer = create_players(0)
        computer_movement(board, computer)
        computer_goes_next = False

    while not victory(board):
        if is_head():
            if computer_goes_next:
                computer_movement(board, computer)
            else:
                human_movement(board, human)
        else:
            computer_goes_next = not computer_goes_next
            if computer_goes_next:
                computer_movement(board, computer)
            else:
                human_movement(board, human)
